...
public class JurosCompostosServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            double vp, porcentagem = 0;
            int meses = 0;
            try{
                vp = Double.parseDouble(request.getParameter("vp"));
                porcentagem = Double.parseDouble(request.getParameter("porcentagem"));
                meses = Integer.parseInt(request.getParameter("meses"));
                double vf = vp*(Math.pow(1+porcentagem/100, meses));
                out.println("<h2>Valor presente = "+vp+"</h2>");
                out.println("<h2>Taxa de juros = "+porcentagem+"</h2>");
                out.println("<h2>Tempo (em meses) = "+meses+"</h2>");
                out.println("<h2>Valor final = "+vf+"</h2>");
            }catch(Exception ex){
                    out.println("<h2 style='color: red'>Erro ao ler o(s) parâmetro(s): "+ex.getMessage()+"</h2>");
            }
            out.println("</body>");
            out.println("</html>");
        }
